...
def setup(direction: str, suitable: str = "是") -> HistoricalSetup:
...
def test_direction_is_derived_without_changing_suitability():
    assert derive_entry_direction(setup("上升"), 89) == "做多"
    assert derive_entry_direction(setup("下降"), 111) == "做空"
    assert derive_entry_direction(setup("上升", suitable="否"), 89) is None
